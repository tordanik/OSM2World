...
/**
 * Parses OSM JSON files. Supports both OSM API and Overpass API dialects.
 * (see the <a href="https://wiki.openstreetmap.org/wiki/OSM_JSON">OSM Wiki</a> and
 * <a href=https://dev.overpass-api.de/output_formats.html#json>Overpass API documentation</a>)
 */
public abstract class JsonReader implements OSMDataReader {
	...
	public OSMData getAllData() throws IOException {
		...
		try {
			...
			json = normalizeVersionToString(json);

			OsmJson root = JsonUtil.fromJson(json, OsmJson.class);

			if (!"0.6".equals(root.version)) {
	...
	/**
	 * Replaces numeric version attributes with strings
	 * (necessary because of differences between OSM API and Overpass API).
	 */
	private String normalizeVersionToString(String json) {
