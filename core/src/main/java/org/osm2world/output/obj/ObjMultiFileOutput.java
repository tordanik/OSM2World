...
/**
 * specialized alternative to {@link ObjOutput} which can split a large scene across multiple obj files.
 * It starts a new file whenever a threshold is reached.
 */
public class ObjMultiFileOutput extends FaceOutput {
	...
	private ObjOutput currentTarget;
	private int primitivesInCurrentTarget = 0;

	/* iterator which creates and wraps .obj files as needed */
	private final Iterator<ObjOutput> objTargetIterator = new Iterator<>() {
		...
		public ObjOutput next() {
			...
			try {
				...
				ObjOutput.writeObjHeader(objWriter, mapProjection);

				objWriter.println("mtllib " + mtlFile.getName() + "\n");

				var target = new ObjOutput(objWriter, mtlWriter, objDirectory, "part_obj");
	 ...
	 */
	public ObjMultiFileOutput(File objDirectory, @Nullable MapProjection mapProjection, int primitiveThresholdPerFile) throws IOException {
		...
		ObjOutput.writeMtlHeader(mtlWriter);
