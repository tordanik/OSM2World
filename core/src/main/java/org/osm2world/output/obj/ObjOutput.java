...
/**
 * Writes models to the Wavefront OBJ format.
 * Also creates the .mtl files which go along with .obj files.
 */
public class ObjOutput extends FaceOutput {
	...
	/**
	 * creates an {@link ObjOutput} which writes to files on the disk.
	 ...
	 */
	public ObjOutput(File objFile, @Nullable MapProjection mapProjection) throws IOException {
	...
	/**
	 * creates an {@link ObjOutput} which writes to {@link Writer}s, not necessarily {@link File}s.
	 *
	 ...
	 */
	public ObjOutput(Writer objWriter, @Nullable Writer mtlWriter,
