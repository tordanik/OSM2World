...
import static org.osm2world.output.jogl.JOGLOutputFixedFunction.drawPrimitive;
...
public class JOGLRendererVBOFixedFunction extends JOGLRendererVBO {
	...
	public void render(final Camera camera, final Projection projection) {
		...
		for (int t = 0; t < JOGLOutputFixedFunction.MAX_TEXTURE_LAYERS; t++) {
			gl.glClientActiveTexture(JOGLOutputFixedFunction.getGLTextureConstant(t));
		...
		for (PrimitiveWithMaterial p : transparentPrimitives) {
			...
			if (!p.material.equals(previousMaterial)) {
				JOGLOutputFixedFunction.setMaterial(gl, p.material, textureManager);
				previousMaterial = p.material;
			}

			drawPrimitive(gl, AbstractJOGLOutput.getGLConstant(p.primitive.type),
